...
const TestRef = () => {
    ...
    const deleteImgBox = index => {
        const deleteExcute = myImage.filter((item, idx) => idx !== index );  //사용자가 삭제버튼을 클릭하지 않은 곳만 필터링한다.
    ...
    return (
        <>
            <h3>이미지 파일 넣기</h3>
            <button onClick={handleFileBtnClick}>[button]이미지 추가하기</button>
